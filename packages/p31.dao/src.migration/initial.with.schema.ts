...
// 실행
// ts-node packages/p31.dao/src.migrations/initial.ts
...
async function up(knex: Knex, schemaName:string) {
    ...
    // User
    await knex.schema.withSchema(schemaName).createTable('User', table => {
    ...
    // Address
    await knex.schema.withSchema(schemaName).createTable('Address', table => {
        // const columns = schema(table)
        // columns.primaryUuid()
    ...
    // Event
    await knex.schema.withSchema(schemaName).createTable('Event', table => {
        // const columns = schema(table)
        // columns.primaryUuid()
